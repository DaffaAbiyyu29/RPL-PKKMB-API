        ...
        public ResponseModel updateMahasiswa([FromBody] MahasiswaBaruModel mhsBaru)
        {
            try
            {
                string query = "UPDATE pkm_msmahasiswa " +
                               "SET mhs_statuskelulusan = @p2 " + // Ubah hanya statuskelulusan
                               "WHERE mhs_nopendaftaran = @p1";
                SqlCommand command = new SqlCommand(query, _connection);
                command.Parameters.AddWithValue("@p1", mhsBaru.mhs_nopendaftaran);
                command.Parameters.AddWithValue("@p2", mhsBaru.mhs_statuskelulusan); // Hanya statuskelulusan
                ...
                response.status = 200;
                response.messages = "Status kelulusan mahasiswa berhasil diubah";
            ...
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message);
                response.status = 500;
                response.messages = "Terjadi kesalahan saat mengubah status kelulusan mahasiswa = " + ex.Message;
                response.data = null;
            }
